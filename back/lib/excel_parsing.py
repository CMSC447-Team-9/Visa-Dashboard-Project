...
def get_excel(file):
    ...
    if ("Start date" in excel.columns):
        ...
        excel.loc[mask, "Start date"] = (
            excel.loc[mask, "Start date"].str.extract(r'(\d{1,2}[/]\d{1,2}[/]\d{2,4})')[0])
        excel["Start date"] = pd.to_datetime(
            excel["Start date"], errors='coerce')
    return excel
...
def current_visas(excel):
    ...
    excel_sorted = excel_sorted.drop_duplicates(
        subset=["Last name", "First Name"], keep='first')
    ...
    excel_sorted.loc[mask, "clean exp date"] = (
        excel_sorted.loc[mask, "clean exp date"].str.extract(r'(\d{1,2}[/]\d{1,2}[/]\d{2,4})')[0])
    excel_sorted["clean exp date"] = pd.to_datetime(
        excel_sorted["clean exp date"], errors='coerce')
    # copies expiration date column to clean exp date column and then converts clean exp date to a datetime
    excel_sorted = excel_sorted[(excel_sorted["clean exp date"].isna()) | (
        excel_sorted["clean exp date"] >= today)]
    # deletes any expired visa rows
    excel_sorted = excel_sorted[~excel_sorted["Expiration Date"].astype(
        str).str.contains(r"\?", na=False)]
    ...
    for _, row in excel.iterrows():
        ...
        if pd.isna(row["clean exp date"]):
            renew_list.append({
                ...
                "umbc_email": row["Employee's UMBC email"]
            })
...
def pending_visas(excel):
    ...
    excel_pending = excel[excel["Expiration Date"].astype(
        str).str.contains(r"\?", na=False)]
    pending_list = []
...
def get_report_stats(sorted_excel, excel):
    ...
    for _, row in sorted_excel.iterrows():
        ...
        if (unknown_department or unknown_gender):
            person_rows = excel[(excel["Last name"] == row["Last name"]) & (
                excel["First Name"] == row["First Name"])].copy()
            person_rows = person_rows.sort_values(
...
def get_period_stats(excel):
    ...
    stats = {"f-1_calendar": 0, "f-1_reporting": 0, "f-1_academic": 0,
             "j-1_calendar": 0, "j-1_reporting": 0, "j-1_academic": 0,
             "h-1b_calendar": 0, "h-1b_reporting": 0, "h-1b_academic": 0,
             "pr_calendar": 0, "pr_reporting": 0, "pr_academic": 0, }
    ...
    for _, row in excel.iterrows():
        ...
        if (academic_start <= row["Start date"] <= academic_end):
            if case_string.startswith("f-1"):
                stats["f-1_academic"] += 1
            elif case_string.startswith("j-1"):
                stats["j-1_academic"] += 1
            elif case_string.startswith("h-1b"):
                stats["h-1b_academic"] += 1
            elif case_string.startswith("permanent"):
                stats["pr_academic"] += 1

        if (reporting_start <= row["Start date"] <= reporting_end):
            if case_string.startswith("f-1"):
                stats["f-1_reporting"] += 1
            elif case_string.startswith("j-1"):
                stats["j-1_reporting"] += 1
            elif case_string.startswith("h-1b"):
                stats["h-1b_reporting"] += 1
            elif case_string.startswith("permanent"):
                stats["pr_reporting"] += 1

        if (calendar_start <= row["Start date"] <= calendar_end):
            if case_string.startswith("f-1"):
                stats["f-1_calendar"] += 1
            elif case_string.startswith("j-1"):
                stats["j-1_calendar"] += 1
            elif case_string.startswith("h-1b"):
                stats["h-1b_calendar"] += 1
            elif case_string.startswith("permanent"):
                stats["pr_calendar"] += 1
    return stats
...
def get_notes(email, excel):
    ...
    notes = "\n".join(
        excel[(excel["Employee's UMBC email"] == email)
              ]["General notes"].dropna().astype(str)
    )
    return notes
...
def get_pr_notes(email, excel):
    ...
    notes = "\n".join(
        excel[(excel["Employee's UMBC email"] == email)
              ]["Permanent residency notes"].dropna().astype(str)
    )
    return notes
...
def add_college(excel):
    ...
    excel["college"] = excel["Department"].str.lower().map(
        dept_to_college).fillna("Unknown")
    return excel
